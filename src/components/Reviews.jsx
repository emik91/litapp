...
function Review({ title, body, author, rating, className, ...props }) {
  ...
  return (
    <figure
      className={clsx(
        'animate-fade-in rounded-3xl bg-white bg-slate-900 p-6 opacity-0 shadow-md shadow-gray-900/5',
        className
      )}
    ...
    >
      <blockquote className="text-gray-900">
        ...
        <p className="mt-4 text-lg font-semibold leading-6 text-gray-50 before:content-['“'] after:content-['”']">
          {title}
        </p>
        <p className="mt-3 text-base leading-7 text-gray-100">{body}</p>
      </blockquote>
      <figcaption className="mt-3 text-sm text-gray-50 before:content-['–_']">
        {author}
      </figcaption>
    </figure>
  )
...
function ReviewGrid() {
  ...
  return (
    ...
    >
      ...
      <div className="pointer-events-none absolute inset-x-0 top-0 h-32 bg-gradient-to-b from-slate-800" />
      <div className="pointer-events-none absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-slate-800" />
    </div>
  )
}
